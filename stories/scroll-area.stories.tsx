...
export const HorizontalOnly: Story = {
  ...
  render: () => (
    <ScrollArea
      ...
      className='border-border h-32 w-full max-w-xl rounded border shadow'
    >
      <div className='flex gap-4 p-4'>
        {Array.from({ length: 20 }, (_, i) => i + 1).map((num) => (
          <div
            key={num}
            className='bg-dimmed/20 flex size-20 shrink-0 items-center justify-center rounded font-semibold'
          >
            {num}
          </div>
        ))}
